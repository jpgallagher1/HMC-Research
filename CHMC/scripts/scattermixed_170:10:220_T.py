...
for i in range(num_perturbs):
    ...
    plt.loglog(lens, mean_chmc_w1_metric[i], '-*',
               label=f'avg CHMC-κ={conds[i]: 0.1f}, slope: {chmc_p[0]: 0.2f}', alpha=1)

plt.gca().set_prop_cycle(None)

for i in range(num_perturbs):
    hmc_p = np.polyfit(np.log(lens), np.log(mean_hmc_w1_metric[i]), 1)
    plt.loglog(lens, mean_hmc_w1_metric[i], '-o',
               label=f'avg HMC-κ={conds[i]: 0.1f}, slope: {hmc_p[0]: 0.2f}', alpha=1)
...
figpath1 = f'plots/avg_w1_scaling_vs_kappa{κ_min}:{κ_max}_T{T}_oneplot2.png'
...
for k, val in enumerate(conds):
    max_chmc_samples = min(2000, chmc_chains[(k,8,0)].shape[0])
    max_hmc_samples = min(2000, hmc_chains[(k,8,0)].shape[0])
    try: 
    ...
    try:    
        ...
        xhmc, yhmc = hmc_chain[(k, 8,0)][:max_hmc_samples,:].T
    except:
        xhmc, yhmc = [],[]
    heatmapchmc, xedgeschmc, yedgeschmc = np.histogram2d(xchmc, ychmc, bins=90)
    heatmaphmc, xedgeshmc, yedgeshmc = np.histogram2d(xhmc, yhmc, bins=90)
    extent = [max(xedgeschmc[0],xedgeshmc[0]), max(xedgeschmc[-1], xedgeshmc[-1]), max(yedgeschmc[0],yedgeshmc[0]),  max(yedgeschmc[-1], yedgeshmc[-1])]
    ...
    plt.clf()
    plt.scatter(xchmc,ychmc, marker='+',c='orange', alpha=0.05, label='samples')
    ...
    plt.scatter([],[], c='cyan', marker='+', label = 'HMC-No Samples')
    plt.legend().legend_handles[1].set_alpha(1)
    subtitle1 = f'\n$dim$ = {dims[0]}, $\\tau = $ {config.τ}, $N = $ {config.N}, κ={conds[0]}'
    plt.title('HMC/CHMC Contour and Histogram plot\n2_000 Samples'+subtitle1)
    plt.legend().set_loc('upper left')

    plt.show()

figpath2 = f'plots/avg_w1_scaling_vs_kappa{κ_min}:{κ_max}_T{T}_oneplot2_grid.png'
