...
dims = jnp.logspace(d_min, d_max, d_steps)
num_dims = len(dims)
dim = dims[0]
...
config = IntegratorConfig(τ=tau, T=T, N=N, tol=tol, max_iter=max_iter, integrator='AVF_NewtonFPI_T')
Mass_inv = jnp.eye(dim)
true_cov_mats = jax.vmap(jnp.eye)(dims)
...
for k, val in enumerate(num_dims):
    target = gen_p_gauss_pdf(β)
    H = gaussian_hamiltonian(target, mass_inv=Mass_inv)
    H_flat = lambda qp_flat, H=H: H(QP.from_array(qp_flat))
    true_cov_k = true_cov_mats[k]
    print(f"{'#'*60}\n# dim: {dim}, num_dim: {k+1} of {num_dims}", f"tau: {config.τ}, T: {config.T}, N: {config.N}")
...
figpath1 = f'plots/avg_cov_max_diag_err_scaling_vs_kappa{d_min}:{d_max}_τ{tau_str}_oneplot2.png'
...
for i, val in enumerate(conds):
    plt.sca(axes[i])
    gen_τ_plots(lens, chmc_cov_metric, hmc_cov_metric, config, i, dims=dims, cond= val, slope = True, avg=True)
figpath2 = f'plots/avg_cov_max_diag_err_scaling_vs_kappa{d_min}:{d_max}_τ{tau_str}_oneplot2_grid.png'
