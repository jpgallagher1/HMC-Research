...
hmc_w1_metric = np.zeros((num_perturbs, mcmc_iter_count, runs))
chmc_w1_metric = np.zeros((num_perturbs, mcmc_iter_count, runs))
...
for k, val in enumerate(perturb_vals):
    ...
    print(f"{'#'*60}\n# dim: {dim}, tau: {config.τ}, T: {config.T}, N: {config.N}, "f"num_perturb_val: {k+1} of {num_perturbs}")
...
figpath1 = f'plots/avg_w1_scaling_vs_kappa{κ_min}:{κ_max}_τ{tau_str}_oneplot2.png'
...
figpath2 = f'plots/avg_w1_scaling_vs_kappa{κ_min}:{κ_max}_τ{tau_str}_oneplot2_grid.png'
