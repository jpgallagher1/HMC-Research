...
# I have been using Σ = [[1, rho], [rho, 1]], which has eigenvalues 1+rho, 1-rho, and so I’ve been using the short hand of cond = (1-rho)/(1+rho) to pick the corresponding perturbation: rho = (cond-1)/(1+cond)

# In the hamiltonian, the corresponding precision matrix is Σ^-1. Given equation 8 from Pourzanjani and Petzold, the stability of LF and the hamiltonian depends on 2/sqrt( spectral radius(Σ^-1)), which for the matrices below would correspond to h < approx 0.396.
...
# Derived quantities
# =============================================================================
κs = jnp.logspace(κ_min,κ_max, κ_steps, base=base2)
...
for k, val in enumerate(perturb_vals):
    ...
    for j, mainnum_samples in enumerate(lens):
        ...
        print(f"{'#'*60}\n# dim: {dim}, tau: {config.τ}, N: {config.N}, "
              f"num_perturb_val: {k} of {num_perturbs}, len: {n_samples}")
...
plt.savefig('plots/avg_cov_max_diag_err_scaling_vs_kappa2^4-2^10_oneplot.png')
print('plot saved')